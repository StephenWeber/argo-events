...
func (rc *Router) HandleRoute(writer http.ResponseWriter, request *http.Request) {
	...
	if len(request.Header["Content-Type"]) > 0 && request.Header["Content-Type"][0] == "application/x-www-form-urlencoded" {
		...
		if err != nil {
			logger.Desugar().Error("failed to process the interaction", zap.Error(err))
		...
		if err != nil {
			logger.Desugar().Error("failed to handle the event", zap.Error(err))
			common.SendInternalErrorResponse(writer, err.Error())
			return
		}
		if response != nil {
			...
			if _, err := writer.Write(response); err != nil {
				logger.Desugar().Error("failed to write the response for url verification", zap.Error(err))
